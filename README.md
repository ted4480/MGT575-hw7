...
Python pipeline: **PDF slide deck → structured JSON agents → per-slide MP3 → one narrated MP4 (ffmpeg)**. Speaking style is derived from `instructor_lecture_transcript.txt` (repo root) and stored in `style.json` for the narration agent.
...
- `instructor_lecture_transcript.txt` — instructor captions/transcript for the **style agent** (repo root; replace with your real transcript if the course provides one)
...
1. **Scaffold & contracts** — “Create repo layout per HW7: `run_lecture_pipeline.py`, `lecture_agents/` package, `requirements.txt`, `.gitignore` (exclude PNG/MP3/MP4), `projects/`, README. Add `instructor_lecture_transcript.txt` at repo root as the style source. Document keys via `--keys-file` pointing at `Gen AI/API_Key.rtf`, no `.env`.”
